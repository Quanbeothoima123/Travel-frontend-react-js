...
const generateTOCFromHtml = (html, options = {}) => {
  ...
  if (!headings.length) {
    return { tocHtml: "", contentHtml: html };
  }
  ...
  // Add smooth scroll behavior if enabled
  const addSmoothScrollScript = () => {
    if (!smoothScroll) return "";

    return `
      <script>
        document.addEventListener('DOMContentLoaded', function() {
          const tocLinks = document.querySelectorAll('.toc-link[data-smooth="true"]');
  ...
  const scriptHtml = addSmoothScrollScript();

  return {
    tocHtml: tocHtml + scriptHtml,
