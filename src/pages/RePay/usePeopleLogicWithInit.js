...
export function usePeopleLogicWithInit({
  seats,
  discountedBase,
  hasAdditional,
  additionalMapById,
  ...
  // State với initial values
  const [baseCounts, setBaseCounts] = useState(initialBaseCounts);
  const [exceedCounts, setExceedCounts] = useState(initialExceedCounts);
  ...
  // Method để update baseCounts và exceedCounts từ bên ngoài
  const updateBaseCounts = useCallback((newCounts) => {
    setBaseCounts((prev) => ({
      ...prev,
      ...newCounts,
    }));
  ...
  const updateExceedCounts = useCallback((newCounts) => {
    setExceedCounts((prev) => ({
      ...prev,
      ...newCounts,
    }));
