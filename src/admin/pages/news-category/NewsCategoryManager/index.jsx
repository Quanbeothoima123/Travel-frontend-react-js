...
import SearchBarNewCategory from "./SearchBarNewsCategory";
import NewsCategoryTree from "./NewsCategoryTree/index.jsx";
import "./NewsCategoryManager.css";
...
const NewsCategoryManager = () => {
  ...
  const handleDeleteConfirm = async (id) => {
    try {
      const res = await fetch(
        `http://localhost:5000/api/v1/admin/news-category/delete/${categoryId}`,
        {
          method: "DELETE",
          headers: {
  ...
  const triggerHighlight = (id) => {
    ...
    const event = new CustomEvent("ncm-expand-to-target", {
      detail: { targetId: id },
    });
  ...
  const handleToggleCollapse = () => {
    const event = new CustomEvent("ncm-toggle-collapse");
    window.dispatchEvent(event);
  };
  ...
  return (
    <div className="ncm-container">
      <h2 className="ncm-title">Quản lý danh mục mới</h2>
      ...
      <div className="ncm-tree-wrapper">
        <NewsCategoryTree
